...
public class CameraPerspectiveEditorEditor : Editor
{
	...
	void OnSceneGUI ()
	{
		if (thisCamera.enabled && cameraOffsetController.enabled)
		{
			...
			if (thisCamera.orthographic)
			{
				originalMatrix.m00 = 1f / thisCamera.aspect;
				originalMatrix.m11 = 1f;
				
				//Begin - Fix for representing orthographic zoom correctly
				originalMatrix.m02 = lensShift.vector2Value.x / thisCamera.aspect;
				originalMatrix.m12 = lensShift.vector2Value.y;
			...
			originalMatrix = thisCamera.cameraToWorldMatrix * originalMatrix.inverse;
			
			bl1 = originalMatrix.MultiplyPoint(-tr);
			tl1 = originalMatrix.MultiplyPoint(-br);
			br1 = originalMatrix.MultiplyPoint(-tl);
			tr1 = originalMatrix.MultiplyPoint(-bl);
			bl2 = originalMatrix.MultiplyPoint(bl);
			tl2 = originalMatrix.MultiplyPoint(tl);
			br2 = originalMatrix.MultiplyPoint(br);
			tr2 = originalMatrix.MultiplyPoint(tr);
